...
class AntColonyOpt:
    def __init__(self, file):
        ...
        self.__solution_generator = SolutionGenerator(self.__file, self.__data)
        # RESET
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        colony = AntColonyOpt(sys.argv[1])
        ...
        ant_group = 25
        tm = time.time()
        colony.calculate(3000, ant_group, 0.50, 5 * ant_group, Method.Invert, (5, 1), True, 500)
        tm = time.time() - tm
        print('Processing time: ' + tm.__str__())
        print('Output: ' + colony.get_solution().__str__())
